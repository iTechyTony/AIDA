...
class AIDARegister {
    ...
    public function forgotPassword($userEmail) {
        ...
        if($userEmail == "")
            $errors[] = AIDALang::get('email_required');
        if( ! $validator->emailValid($userEmail) )
            $errors[] = AIDALang::get('email_wrong_format');
        
        if( ! $validator->emailExist($userEmail) )
            $errors[] = AIDALang::get('email_not_exist');

        $login = new AIDALogin();

        if($login->_isBruteForce())
            $errors[] = AIDALang::get('brute_force');
    ...
    public function validateUser($data, $botProtection = true) {
        ...
        if( $validator->isEmpty($user['email']) )
            $errors[] = array( 
                "id"    => $id['email'],
                "msg"   => AIDALang::get('email_required')
            );
        ...
        if( $validator->isEmpty($user['password']) )
            $errors[] = array( 
                "id"    => $id['password'],
                "msg"   => AIDALang::get('password_required')
            );
        ...
        if($user['password'] != $user['confirm_password'])
            $errors[] = array( 
                ...
                "msg"   => AIDALang::get('passwords_dont_match')
            );
        ...
        if( $validator->usernameExist($user['username']) )
            $errors[] = array( 
                "id"    => $id['username'],
                "msg"   => AIDALang::get('username_taken')
            );
