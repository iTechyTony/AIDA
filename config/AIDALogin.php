...
class AIDALogin {
    ...
    public function byId($id) {
        if ( $id != 0 && $id != '' && $id != null ) {
            ...
            AIDASession::set("user_id", $id);
            if(LOGIN_FINGERPRINT == true)
                AIDASession::set("login_fingerprint", $this->_generateLoginString ());
        }
    }
    ...
    public function isLoggedIn() {
        ...
        if(AIDASession::get("user_id") == null)
             return false;
        
        //if enabled, check fingerprint
        if(LOGIN_FINGERPRINT == true) {
            $loginString  = $this->_generateLoginString();
            $currentString = AIDASession::get("login_fingerprint");
    ...
    public function userLogin($username, $password) {
        ...
        if(count($result) == 1) 
        {
            ...
            if($result[0]['confirmed'] == "N") {
                echo AIDALang::get('user_not_confirmed');
                return false;
            }
            ...
            $this->increaseLoginAttempts();
            echo AIDALang::get('wrong_username_password');
            return false;
        }
    }
    ...
    private function _validateLoginFields($username, $password) {
        ...
        if($username == "")
            $errors[] = AIDALang::get('username_required');
        
        if($password == "")
            $errors[] = AIDALang::get('password_required');
